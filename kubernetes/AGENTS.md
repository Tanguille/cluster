...
- Use `${SECRET_DOMAIN}` for URLs; never hardcode domains. Exception: chart
  sources (`HelmRepository.spec.url`, `OCIRepository` URLs) point at upstream
  chart publishers, not the cluster domain — keep them as published.
