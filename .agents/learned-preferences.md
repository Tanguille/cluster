...
- **Push/PR atomicity:** treat push + open/advance PR as ONE step (one `github_push_files` + PR create/update sequence via ToolHive). If the finish cannot be done in the current turn, write `HANDOFF.md` + `PLAN.md` per the `handoff` skill BEFORE stopping — never leave "push + PR" implicit in chat; a session boundary will orphan it.
- **All GitHub PR work goes through ToolHive `github_*` tools** (2026-09-07, user standing order — "use github tools via toolhive mcp"): `github_list_pull_requests`, `github_pull_request_read` (methods: get/get_status/get_check_runs/get_comments/get_review_comments/get_diff/get_files/get_commits), `github_update_pull_request`, `github_push_files` (full file contents, no delete), `github_delete_file`, `github_create_branch`, `github_update_pull_request_branch` (server-side rebase — primary Gate A). No local `git push`: this box has no `gh`/`GITHUB_TOKEN` and the `pushremote` mirror is dead. Sole exception: a local rebase-conflict resolution, then the last-resort route (worktree commit → `git bundle` → `scp` to server → fetch+push from `~/cluster` via its authed gh). Details: `.agents/skills/pr-review/references/pr-shepherd.md`.
