...
### Revalidation never works for LFS weights (2026-07-27)

Every restart of an InferenceService re-downloads the full weight set. Measured on
`qwen36-27b`: two consecutive restarts each re-fetched all 17,388,134,707 bytes of
`model.safetensors`, at 11 and 20 MB/s, so a "restart" costs 15-25 minutes rather than the
seconds the revalidation path implies.

Small non-LFS files revalidate correctly. Seeding an etag file and re-requesting
`tokenizer_config.json` gives `http=304 size=0` on runs 2 and 3, with the etag preserved.
...
`huggingface.co/.../resolve/<rev>/<file>` redirects to a signed CDN URL on a different
host, and the conditional request does not survive that hop, so the CDN streams the whole
object. `--etag-compare` can therefore only ever short-circuit the small config files,
never the weights.

Note the interaction with the corruption hazard above: point 2 assumes a `304` can be
served over a truncated weight file. For LFS artifacts that cannot happen, since they are
...
upstream fix: `HEAD` and compare `Content-Length` before transferring, download to
`$dest.tmp` + `mv`, and log `revalidated` only on an actual `304`. Filed as
